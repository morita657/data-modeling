...
def process_log_file(cur, filepath):
    ...
    # timestamp
    time_data = (timestamp, t.dt.hour, t.dt.day, t.dt.weekofyear, t.dt.month, t.dt.year, t.dt.weekday)

    column_labels = ["start_time", "hour", "day", "week of year", "month", "year", "weekday"]
    d = pd.Series((time_data), index=column_labels).to_dict()
    ...
    d = pd.Series((user_data), index=column_labels).to_dict()
    user_df = pd.DataFrame({c: d for c,d in zip (column_labels, user_data)}).dropna()
